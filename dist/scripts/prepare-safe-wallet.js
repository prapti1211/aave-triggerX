...
const abis_1 = require("../src/contracts/abis");
/**
...
async function prepareSafeWallet() {
    ...
    console.log('─'.repeat(60));
    const safeAddress = config_1.config.safeWalletAddress;
    if (!safeAddress) {
    ...
    if (!ethers_1.ethers.isAddress(safeAddress)) {
    ...
    try {
        ...
        owners.forEach((owner, index) => {
            ...
            if (owner.toLowerCase() === config_1.config.userAddress.toLowerCase()) {
    ...
    console.log('\nStep 3: Check WETH Balance in Safe Wallet');
    console.log('─'.repeat(60));
    const wethContract = new ethers_1.ethers.Contract(abis_1.WETH_ADDRESS, abis_1.ERC20_ABI, provider);
    try {
        const wethBalance = await wethContract.balanceOf(safeAddress);
        const wethBalanceFormatted = ethers_1.ethers.formatEther(wethBalance);
        ...
        if (wethBalance === 0n) {
            ...
            const estimatedTopups = Math.floor(Number(wethBalanceFormatted) / Number(ethers_1.ethers.formatEther(config_1.config.topUpAmount)));
    ...
    console.log('WETH Balance:', ethers_1.ethers.formatEther(await wethContract.balanceOf(safeAddress)), 'WETH');
    console.log('ETH Balance:', ethers_1.ethers.formatEther(await provider.getBalance(safeAddress)), 'ETH');
    console.log('Monitored User Address:', config_1.config.userAddress);
    console.log('Aave Pool Address:', config_1.config.aave.poolAddress);
    console.log('\nNext Steps:');
    console.log('─'.repeat(60));
    ...
    console.log('─'.repeat(60));
    console.log('• Monitor health factor of:', config_1.config.userAddress);
    console.log(`• Trigger when health factor drops below ${config_1.config.healthFactorThreshold}`);
    console.log(`• Supply ${ethers_1.ethers.formatEther(config_1.config.topUpAmount)} WETH per top-up`);
...
//# sourceMappingURL=prepare-safe-wallet.js.map